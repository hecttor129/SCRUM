...
namespace VISTA.UserControls
{
    public partial class DashboardEquipoControl : UserControl
    {
        ...
        {
            if (dgArchivos.SelectedItem is not ArchivoDto sel)
            {
            ...
            try
            {
                ...
                panelCargandoIAEquipo.Visibility = Visibility.Visible;
                panelResultadoIAEquipo.Visibility = Visibility.Collapsed;
                btnAnalizarIAEquipo.IsEnabled = false;

                // Enviar al webhook de n8n
                string resultado = await _n8nService.AnalizarDocumentoAsync(rutaTemporal);

                // Mostrar resultado organizado
                panelCargandoIAEquipo.Visibility = Visibility.Collapsed;
                panelResultadoIAEquipo.Visibility = Visibility.Visible;
                txtArchivoAnalizadoEquipo.Text = $"— Archivo: {sel.NombreOriginal}";
                txtResultadoIAEquipo.Text = resultado;
            }
            catch (Exception ex)
            {
                panelCargandoIAEquipo.Visibility = Visibility.Collapsed;
            ...
            {
                btnAnalizarIAEquipo.IsEnabled = true;
            }
        }
